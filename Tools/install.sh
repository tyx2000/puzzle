#!/bin/bash
# Install Gift.app and the `gift` command.
#
#   ./Tools/install.sh
#
# Copies build/Gift.app to /Applications (falling back to ~/Applications if
# that isn't writable) and installs `gift` into a directory that is on PATH.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BUILD="$ROOT/build/Gift.app"
...
rm -rf "$APPDIR/Gift.app"
cp -R "$BUILD" "$APPDIR/Gift.app"
echo "installed $APPDIR/Gift.app"
...
[ -x "$LSREGISTER" ] && "$LSREGISTER" -f "$APPDIR/Gift.app" >/dev/null 2>&1
...
# user's Terminal never sees, which silently installs `gift` somewhere their
...
sed "s|^DEV_BUILD=\"\"|DEV_BUILD=\"$BUILD\"|" "$ROOT/Tools/gift" > "$BINDIR/gift"
chmod +x "$BINDIR/gift"
echo "installed $BINDIR/gift"
