...
enum RegressionTests {
    ...
    private static func testGitMarkPulls() throws {
        ...
        try expect(pulled == 1 && selected == 0,
                   "a click on the Git mark did not pull, or did more: \(pulled) \(selected)")
        // The branch name stays a label: a click on it is a click on the row.
        let branchBox = row.branchRectForTesting
        row.pressForTesting(at: NSPoint(x: branchBox.midX, y: branchBox.midY))
        row.pressForTesting(at: NSPoint(x: 30, y: branchBox.midY))
        try expect(pulled == 1 && selected == 2,
                   "the branch or the rest of the row pulled, or lost its click")
        ...
        try expect(pulled == 1 && selected == 3, "a click where no mark is drawn pulled")
    ...
    private static func testTimeoutSurvivesAHeldPipe() throws {
        ...
        // The answer must not wait for it, nor be lost to it.
        let finishedAt = Date()
        let done = GitService.runProcess(
            executable: URL(fileURLWithPath: "/bin/sh"),
            arguments: ["-c", "echo fetched; (sleep 5) & exit 0"],
            in: directory, timeout: 60)
        let waited = Date().timeIntervalSince(finishedAt)
        ...
        try expect(waited < GitService.readerGrace + 1,
                   "a finished call waited \(waited)s on what it left in the background")
    }
